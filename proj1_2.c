...
unsigned int inputPassword[4]; // Array to store the input password
unsigned int predefinedPassword[4] = {1, 2, 3, 4}; // Define the pre-defined password
...
void ReadPassword(unsigned int *inputPassword) {
    ...
    while (passwordIndex < PASSWORD_LENGTH)
    {
        while (1)
        {
            ...
            if (key >= '0' && key <= '9' && passwordIndex < PASSWORD_LENGTH) {
                ...
                inputPassword[passwordIndex++] = key - '0'; // Convert char to int and store
                ...
                for (int j = 0; j < PASSWORD_LENGTH; j++) {
                    lcd_comdata(0xc0 + j, 0); // Move cursor to the next position
                    lcd_puts(inputPassword[j]);
                }
